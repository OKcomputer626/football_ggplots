...
```{r}
page1 <- "https://www.transfermarkt.us/los-angeles-fc/leistungsdaten/verein/51828/plus/0?reldata=MLS1%262020"
...
```{r}
page <- "https://www.transfermarkt.us/los-angeles-fc/kader/verein/51828/plus/1/galerie/0?saison_id=2020"
...
# grab joined team
joinedteam <- scraped_page %>% html_nodes("td:nth-child(8)") %>% html_text()

# grab leave team
leaveteam <- scraped_page %>% html_nodes("td:nth-child(10)") %>% html_text()
```
...
lafc <- lafc_comb %>%
  mutate(age = as.numeric(age),
         ...
         join = join %>% mdy(),
         join_age = case_when(
           name == "Cristian Arango" ~ 26.2,
           name == "Jamal Blackman" ~ 27.8,
           name == "Sebastien Ibeagha" ~ 19.4,
           name == "Danny Crisostomo" ~ 24.4,
           name == "Michee Ngalina" ~ 21.3,
...
lafc_plot <- lafc %>% 
    ...
    labs(
      x = "Age", 
      y = "Minutes Played (%)",  
      title = "<span style = 'color: #c39e6d' > Los Angeles Football Club </span> | Squad Age Profile",
      subtitle = "Major League Soccer 2021 (100% = 3060 total Minutes)") +
    theme_bw() +
    theme(
      ...
      plot.title = element_markdown(color = "white", size = 20, face = "bold"),
      plot.subtitle = element_text(color = "white", size = 18),
      plot.caption = element_text(size = 14),
      panel.grid.minor = element_blank(),
      axis.text.x = element_text(color = "white"),
      axis.title.y = element_text(color = "white", vjust= 0.5),
...
legend_plot <- lafc %>% 
  ...
  geom_rect(data = rect_df,
            aes(x = NULL, y = NULL,
                xmin = xmin, xmax = xmax,
                ymin = ymin, ymax = ymax),
            fill = "#c39e6d", alpha = 0.4) +
  geom_link(data = leg_df,
            aes(x = x, xend = xend,
                y = y, yend = yend,
                alpha = stat(index)), 
            color = "#000000", size = 1.75) + 
  geom_point(x = 28, y = 0.65, color = "#e31837", size = 3.5) +
  ## top arrow
...
ggdraw(plot_grid(lafc_plot, legend_plot, 
                 rel_heights = c(2, 1)))
...
lafc_final_plot <- ggdraw(lafc_plot +
         theme(plot.margin = unit(c(1, 10, 1, 1), "cm"))) +
  draw_plot(legend_plot, .75, 0.05, .2, .3) +
  draw_plot(summary_stats, 0.75,0.37,0.2,0.3)
...
```{r}
ggsave(plot = lafc_final_plot, "C:/Users/16267/Downloads/lafc_age_utility_2021.png",
       height = 8, width = 15)
```

```{r}
plot_logo <- add_logo(plot_path = "C:/Users/16267/Downloads/lafc_age_utility_2021.png",
                      logo_path = "https://upload.wikimedia.org/wikipedia/commons/thumb/8/86/Los_Angeles_Football_Club.svg/800px-Los_Angeles_Football_Club.svg.png",
                      logo_position = "top right",
                      logo_scale = 23)
```

```{r}
image_write(image = plot_logo, "C:/Users/16267/Downloads/lafc_age_utility_2021.png")
```
